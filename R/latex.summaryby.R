...
function (object, title="", INDICES=NULL,file= "", caption="%s",
  ...
  if (file != "") unlink(file)   # delete old tex file  
  isfirst = TRUE # only print out detailed caption on first time
  if (is.null(INDICES)) INDICES = ind
  z = by(object,object[INDICES],function(y) {
     ...
     faknam <-  sapply(byINDICES0,
       function(object)(paste(levels(object)[object])))
     if (onetable) 
       byINDICES <- "" else
       byINDICES <- paste(nam,faknam,sep=" ",collapse=", ")
     labINDICES <- tolower(paste(labINDICES,faknam,sep=".",collapse=""))
     labINDICES <- paste(unique(unlist(strsplit(labINDICES,"(\\.+)"))),
       collapse=".")
     ...
     if (isfirst) {
       if (lang == "en") {
         ...
         caption = paste(caption, q,". \\emph{Med}: median; \\emph{StdDev}: standard deviation; \\emph{StdErr}: standard error of mean.", sep="")
        } else {
         q = ifelse(noQuantiles,"", ". \\emph{Q1, Q3}: Quartile");
         caption = paste(caption, q,". \\emph{Med}: Median; \\emph{StdAbw}: Standardabweichung; \\emph{StdFeh}: Fehler des Mittelwerts.", sep="")
        }
     ...
     xt = lapply(y, function(object) {
         ...
         if (su["n"] >1)
           su["StdErr"] = su["StdDev"]/(sqrt(su["n"]-1)) else
           su["StdErr"] = NA
         su
       })
     xt = do.call(rbind,xt)
     ...
     # degenerate to 0
     ndec = pmax(round(1-log10(apply(xt,1,
        function(object) object[8])+0.000001)+moredec),0)
     # standard error has more decimals
     ncol = ifelse(noQuantiles,4,6)
     dec = cbind(matrix(ndec,nrow=length(ndec),ncol=ncol),0,ndec,ndec+1)
     if (onetable)     {
       ...
       lx = latex(xt,title=title,file=file,label=label,caption=caption,
         booktabs = !ctable,append=TRUE,
         caption.loc="bottom",ctable=ctable,rowlabel=rowlabel,
         center="centering",where=where,
         dec=dec,
         ...)
       if (file =="") invisible(xt) else invisible(lx)
    }
  })
  if (onetable) {  # Merge rows
    ...
    lx = latex(za,title=title,file=file,label=label,caption=z[[1]][[3]],
    ...
    if (file == "") invisible(za)  else invisible(lx)
  } else {
    if (file == "") invisible(z)  else invisible(z[[1]])
  }
}
