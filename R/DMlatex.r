...
#  hascgroup = FALSE
  ...
  pcol = substr(names(object),1,1) == "p" | substr(names(object),1,2) == "Pr"
  ...
  if (!hascgroup && is.null(cellTex) && any(pcol) &&
    ...
    sigp <- object[,pcol,drop=FALSE]< shadep # cells that will be shaded
    cols = colnames(object)[pcol]
...
#  ctable = FALSE # because of a bug in latex 3.8.1, giving additional lines
  ...
  if (usemincap & misscap)  mincapwidth=120
  if (usemincap)
  ...
  # with default row label
    latex(object,ctable=ctable,file=file, na.blank=TRUE,
...
#    object = sapply(object,as.character)
    latex(object,ctable=ctable,file=file, rowname=rowname,na.blank=TRUE,
      longtable=longtable, caption.loc="bottom",label=label,
      rowlabel=rowlabel,cellTexCmds = cellTex,
      caption=caption, where=where, booktabs = !ctable,
      first.hline.double=FALSE,...)
...
DMlatex.summary.formula.reverse = function( object,label,caption,
  rowname=NULL, longtable=FALSE, ctable=FALSE,file="",
  where="!htbp",
  rowlabel="",
  mincapwidth,width,cellTexCmds,lines.page=10000,... ) {
  latex.summary.formula.reverse(object,ctable=ctable,file=file, na.blank=TRUE,
      longtable=longtable, caption.loc="bottom",label=label,
...
DMlatex.summary.formula.cross = function( object,label,caption,
  rowname=NULL, longtable=FALSE, ctable=FALSE,file="",
  where="!htbp",
  rowlabel="",
  mincapwidth,width,cellTexCmds=NULL,lines.page=10000,... ) {
  latex.summary.formula.cross(object,ctable=ctable,file=file, na.blank=TRUE,
      longtable=longtable, caption.loc="bottom",label=label,
      caption=caption, lines.page=lines.page, where=where,
